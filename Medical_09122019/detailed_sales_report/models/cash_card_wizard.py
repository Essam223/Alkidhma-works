...
class CashCardWizard(models.TransientModel):
    _name = "detailed.cash.card.wizard"
    ...
    payment_mode = fields.Many2one('account.journal', 'Payment Mode',
                                   domain=[('invoice_journal', '=', True),
                                           ('type', 'in', ('bank', 'cash'))])
    @api.model
    def default_get(self, fields):
        res = super(CashCardWizard, self).default_get(fields)
    ...
    def send_sale_report(self):
        ...
        report_id = 'detailed_sales_report.sales_report_detailed'
        pdf = self.env.ref(report_id).render_qweb_pdf(self.ids, data=data)
        b64_pdf = base64.b64encode(pdf[0])
        attachment_name = 'Detailed Sales Report: ' + str(self.period_start) + " To " + str(self.period_stop)
    ...
    def sale_report(self):
        _logger.warning("Nnnnnnnnnnnnnnnnnnnnnnn")
        doctor = False
        cashier = False
        if self.cashier:
            cashier = [self.cashier.id, self.cashier.name]
        ...
        return self.env.ref('detailed_sales_report.sales_report_detailed').report_action(self, data=data)


class ReportSale(models.AbstractModel):

    _name = 'report.detailed_sales_report.detailed_sales_report_pdf'

    @api.model
    def get_sale_details(self, date_type=False, period_start=False, period_stop=False, doctor=False, patient=False,
                         payment_mode=False, cashier=False):
        cash = 0
        card = 0
        cash_count = 0
        card_count = 0
        if date_type == 'Invoice Date':
            ...
            for order in orders_new:
                ...
                if order.type == 'out_invoice':
                    order_data = {
                        ...
                        'invoice_line_ids': order.invoice_line_ids,
                    }
                else:
                    amount_total =  0.0
                    ...
                    order_data = {
                        ...
                        'disc_total': disc_total,
                        'due': order.residual_signed,
                        'initial_amt': amount_total + disc_total,
                        'invoice_line_ids': order.invoice_line_ids,
                    }
                order_list.append(order_data)
        else:
            if isinstance(patient, bool):
                pass
            else:
                patientid = self.env['medical.patient'].search([('id', '=', patient[0])])
            dom = [
            ...
            if patient:
                dom.append(('partner_id', '=', patientid.name.id))
            if payment_mode:
            ...
            for payment in payment_records:
                _logger.warning("payment== %r", payment)
                cash = 0
                credit = 0
                pay_mode = False
                journal_obj = self.env['account.journal']
                ...
                if payment.journal_id.id in bank_journals:
                    ...
                    pay_mode = 'card'
                if payment.advance:
                    _logger.warning("advancee  %r", payment)
                    if payment.payment_type == 'inbound':
                        order_data = {
                            ...
                            'patient_id': payment.partner_id.patient_id,
                            'doctor': False,
                            'date_invoice': False,
                            'type': 'out_invoice',
                            'pay_mode': pay_mode,
                    ...
                    _logger.warning("invoice==1 , %r", payment)
                    ...
                    if flag == 0:
                        ...
                        visit = ""
                        if order.appt_id:
                            visit = order.appt_id.name
                            visit = visit.replace('APT', '')
                        if order.type == 'out_invoice':
                            order_data = {
                                'number': order.number,
                                'visit': visit,
                    ...
                    _logger.warning("last elseee %r", payment)
                    if payment.payment_type == 'inbound':
                        order_data = {
                            'number': payment.name,
                            'visit': "",
                    ...
                    order_list.append(order_data)
        _logger.warning("order_listttttt %r", order_list)
        return {
