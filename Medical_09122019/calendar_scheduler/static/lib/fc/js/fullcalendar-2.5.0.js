...
var TimeGrid = FC.TimeGrid = Grid.extend(DayTableMixin, {
	...
	renderSlatRowHtml: function() {
		...
		while (slotTime < this.maxTime) {
			...
			var time_sol_label1 = slotDate.format(this.labelFormat);

            try {
                var time_sol_label = time_sol_label1.split(':');
                if (time_sol_label.length == 1) {
                    var label_type = time_sol_label[0].slice(-2);
                    time_sol_label1 = time_sol_label[0].replace(label_type, ':00' + label_type);
                }
            }
            catch (err) {}
			axisHtml =
