...
ResourceView = (function(superClass) {
  ...
  ResourceView.prototype.getResourceText = function(resource) {
    var res_text = '<span>'+this.getResourceTextFunc()(resource) + '</span>';

    if (resource.dept) {
		res_text += '<br /> <span>' + resource.dname + '</span>';
    }
    return res_text;
  };
...
//    var img_src = '<img class="o_cal_avatar" src="/web/image/res.partner/emp_id/image">';
//    return '<th class="fc-resource-cell"' + (colspan > 1 ? ' colspan="' + colspan + '"' : '') + '>'+ img_src.replace("emp_id", resource.p_id) + htmlEscape(this.view.getResourceText(resource)) + '</th>';
//    return '<th class="fc-resource-cell"' + (colspan > 1 ? ' colspan="' + colspan + '"' : '') + '>' + htmlEscape(this.view.getResourceText(resource)) + '</th>';
    return '<th class="fc-resource-cell"' + (colspan > 1 ? ' colspan="' + colspan + '"' : '') + '>' + this.view.getResourceText(resource) + '</th>';
  },
