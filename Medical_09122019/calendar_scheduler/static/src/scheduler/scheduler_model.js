odoo.define('calendar_scheduler.CalendarModel2', function (require) {
"use strict";
...
return AbstractModel.extend({
    ...
    createRecord: function (event) {
        ...
        for (var i in data) {
            event.data.create_vals[i] = data[i];
        }
        return this._rpc({
                model: 'medical.appointment',
                method: 'create',
                args: [event.data.create_vals],
//                context: event.data.options.context,
    ...
    load: function (params) {
        ...
        this.mode = 'day';       // one of month, week or day
        ...
        if (!this.preload_def) {
            ...
            $.when(
                ...
                this._rpc({model: 'calender.config', method: 'find_doctor_ids'}),
                )
            .then(function (write, create, doctor_ids) {
    ...
    updateRecord: function (record) {
        ...
        var resource_vals = record.resourceId.split('_');
        if (resource_vals[0] == 'doctor') {
            data['doctor'] = parseInt(resource_vals[1]);
        } else if (resource_vals[0] == 'room') {
            data['room_id'] = parseInt(resource_vals[1]);
        }
        else {
            data['doctor'] = parseInt(record.resourceId);
        }
...
//                    titleFormat: time.getLangTimeFormat(),
     ...
     * @returns {Array}
     */
     _findDate: function (dateVal) {
        var dateString = dateVal.utc().format('YYYY-MM-DD HH:mm:ss');
    ...
    _getRangeDomain: function () {
        ...
        var domain = [[this.mapping.date_start, '<=', this._findDate(this.data.end_date)]];
        if (this.mapping.date_stop) {
            domain.push([this.mapping.date_stop, '>=', this._findDate(this.data.start_date)]);
        } else if (!this.mapping.date_delay) {
            domain.push([this.mapping.date_start, '>=', this._findDate(this.data.start_date)]);
        }
        return domain;
    },
    /**
    ...
    _loadCalendar: function () {
		...
		var fieldsList = self.fieldNames;
		fieldsList.push('color',
		                'state',
		                'room_id',
		                'patient', 'patient_file_no');

		var domain_val = self.data.domain.concat(self._getRangeDomain()).concat(self._getFilterDomain());
		if (self.data.state_domain) {
			domain_val.push(self.data.state_domain);
		}
		else {
			domain_val.push(['state',
							'in',
							["draft",
							 "confirmed",
						     "checkin", "ready", "done",
		...
		if (self.doctor_ids) {
			domain_val.push(['doctor', 'in', self.doctor_ids]);
		}
        ...
        return $.when.apply($, defs).then(function () {
            ...
            .then(function (events) {
                ...
                self.data.appointment_patient = {};
                var room_appointments = _.map(
                    events,
                    self._recordToCalendarEvent2.bind(self));

                for (var r in room_appointments) {
                    if (room_appointments[r])
	                    self.data.data.push(room_appointments[r]);
                }
                return $.when(
    ...
    _recordToCalendarEvent: function (evt) {
        ...
        var r = {
            ...
            'id': evt.id,
            'attendees':attendees,
            'resourceId': evt['doctor'] ? 'doctor_' + evt['doctor'][0] : -1,
            'doctor': evt['doctor'] ? evt['doctor'][0] : null,
            'room_id': evt['room_id'] ? evt['room_id'][0] : null,
            'color': evt.color,
            'textColor': 'black',
    ...
    _recordToCalendarEvent2: function (evt) {
        this.data.appointment_patient[evt.id] = [
            evt.patient ? Object.values(evt.patient)[0]:null,
            evt.patient_file_no
            ];
        if (!evt.room_id) {
            return null;
        ...
        var r = {
            ...
            'id': evt.id,
            'attendees':attendees,
            'resourceId': evt['room_id'] ? 'room_' + evt['room_id'][0] : -1,
            'doctor': evt['doctor'] ? evt['doctor'][0] : null,
            'room_id': evt['room_id'] ? evt['room_id'][0] : null,
            'color': evt.color,
            'textColor': 'black',
